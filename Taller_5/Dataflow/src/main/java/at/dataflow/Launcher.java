...
public class Launcher {
    ...
    private static String getSootClassPath() {
        ...
        if (sootClassPath == null) {
            String jreRtPath = Paths.get(System.getProperty("java.home"), "lib", "rt.jar").toString();
            String jarPath = Paths.get(System.getProperty("user.dir"), "build", "libs", "Dataflow-1.0-SNAPSHOT.jar").toString();
            sootClassPath = jarPath + File.pathSeparator + jreRtPath;
        }

        System.out.println("SCP: " + sootClassPath);
    ...
    public static void analyzeClass(String targetClass) {
        String sootClassPath = Launcher.getSootClassPath();
