...
public class PointsToGraph {
    ...
    public void addEdge(Node leftNode, String fieldName, Node rightNode) {
         Axis axis = new Axis(leftNode, fieldName, rightNode);
         this.axis.add(axis);
    }
    ...
    public Set<Node> getReachableNodesByField(Node node, String fieldName) {
        ...
        for (Axis axis : this.axis) {
            if (axis.fieldName.equals(fieldName) && axis.leftNode.equals(node)) {
                reachableNodes.add(axis.rightNode);
            }
        }
    ...
    public void union(PointsToGraph in) {
        ...
        for (String k : in.mapping.keySet()) {

            if (this.mapping.containsKey(k)) {
                this.mapping.get(k).addAll(in.mapping.get(k));
            }
            else {
                this.mapping.put(k, in.mapping.get(k));
            }
        }
    }
}
